...
class AverageMeter(object):#计算和更新数值
...
def strip_state_dict(state_dict, strip_key='module.'):#多gpu计算相关
...
def get_dino_head_weights(pretrain_path):#DINO相关
...
def transform_moco_state_dict(obj, num_classes):#MOCO,resnet
...
def init_experiment(args, runner_name=None, exp_id=None):#实验标记，命名，记录
...
class ClassificationPredSaver(object):#分类预测保存
...
def get_acc_auroc_curves(logdir):#从本地获取准确率曲线
...
class IndicatePlateau(object):#表征达到稳定状态
    ...
    def step(self, metrics, epoch=None):
        ...
        if self.num_bad_epochs > self.patience:#当坏epoch数量达到阈值
