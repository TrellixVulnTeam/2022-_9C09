...
def cluster_acc(y_true, y_pred, return_ind=False):#聚类精度计算
    """
...
def purity_score(y_true, y_pred):#混淆矩阵计算
...
class AverageMeter(object):#计算和存储已有值，和进行数据更新平均
...
def PairEnum(x,mask=None):#枚举全部对
...
def accuracy(output, target, topk=(1,)):#确定的排序结构下的准确率计算
