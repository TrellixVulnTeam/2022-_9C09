import numpy as np
import torch
from joblib.parallel import Parallel, delayed, effective_n_jobs
...
class K_Means:
    def __init__(self, k=3, tolerance=1e-4, max_iterations=100, init='k-means++', n_init=10, random_state=None,
                 n_jobs=None, pairwise_batch_size=None):
        self.k = k
    ...
    def fit_mix_once(self, u_feats, l_feats, l_targets, random_state):
        ...
        l_classes = torch.from_numpy(np.array(list(set(list(l_targets.cpu().numpy()))))).cuda()
        ...
        for it in range(self.max_iterations):
            ...
            center_shift = torch.sum(torch.sqrt(torch.sum((centers - centers_old) ** 2, dim=1)))
            if center_shift ** 2 < self.tolerance:
                # break out of the main loop if the results are optimal, ie. the centers don't change their
    ...
    def fit_mix(self, u_feats, l_feats, l_targets):
        ...
        if effective_n_jobs(self.n_jobs) == 1:
            ...
            results = Parallel(n_jobs=self.n_jobs, verbose=0)(
                delayed(self.fit_mix_once)(u_feats, l_feats, l_targets, seed) for seed in seeds)
            # Get results with the lowest inertia
            labels, inertia, centers, n_iters = zip(*results)
            best = np.argmin(inertia)
            self.labels_ = labels[best]
...
def main():
    ...
    cat_feats = torch.from_numpy(cat_feats).cuda()
    u_feats = torch.from_numpy(u_feats).cuda()
    l_feats = torch.from_numpy(l_feats).cuda()
    l_targets = torch.from_numpy(l_targets).cuda()
